...
export async function registerRoutes(app: Express): Promise<Server> {
  // QR Gen Tool Proxy - FIRST PRIORITY - Register before ANY other middleware
  console.log('[QR Setup] Registering QR Gen Tool proxy routes...');
  
  // Add middleware to log all QR tool requests for debugging
  app.use('/qr-gen-tool*', (req, res, next) => {
    console.log(`[QR Debug] Request: ${req.method} ${req.path} ${req.originalUrl}`);
    next();
  });
  
  // Handle redirect from /qr-gen-tool to /qr-gen-tool/ (EXACT match only - not catching /qr-gen-tool/)
  app.get('/qr-gen-tool$', (req, res) => {
    console.log(`[QR Debug] Redirecting /qr-gen-tool to /qr-gen-tool/`);
    res.redirect(301, '/qr-gen-tool/');
  });
  
  app.get('/qr-gen-tool/', async (req, res) => {
    const targetUrl = 'https://qr-gentool-vjvaibhu.replit.app/';
    
    console.log(`[QR Proxy ROOT] ${req.method} ${req.path} -> ${targetUrl}`);
    ...
    try {
      ...
      console.log(`[QR Proxy] Response: ${response.status} ${response.statusText}, Content length: ${buffer.length} bytes`);
      
      if (contentType.includes('text/html')) {
        let content = buffer.toString('utf-8');
        
        // Rewrite asset paths to work with our proxy
  ...
  console.log('[QR Setup] QR Gen Tool proxy routes registered');
